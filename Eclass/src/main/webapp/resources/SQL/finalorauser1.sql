...
,constraint PK_subject_tbl PRIMARY KEY (subseq)
,constraint FK_subject_tbl foreign key(fk_userid) references eclass_member(userid)
...
,constraint PK_lecture_tbl PRIMARY KEY (lecSeq)
,constraint FK_lecture_tbl foreign key(fk_subSeq) references subject_tbl(subseq)
);
create sequence lecSeq
start with 1
increment by 1
nomaxvalue
nominvalue
...
,constraint FK_lectureComment_tbl foreign key(fk_lecSeq) references lecture_tbl(lecSeq)
,constraint FK_lectureComment_tbl foreign key(fk_userid) references eclass_member(userid)
);
create sequence lecComSeq
start with 1
increment by 1
nomaxvalue
nominvalue
...
,constraint PK_attand_tbl PRIMARY KEY (attandSeq)
,constraint FK_attand_tbl foreign key(fk_subSeq) references subject_tbl(subSeq)
,constraint FK_attand_tbl foreign key(fk_lecSeq) references lecture_tbl(lecSeq)
,constraint FK_attand_tbl foreign key(fk_userid) references eclass_member(userid)
);
create sequence attandSeq
start with 1
increment by 1
nomaxvalue
nominvalue
...
,constraint PK_myPForP_tbl PRIMARY KEY (myPSeq)
,constraint FK_myPForP_tbl foreign key(fk_userid) references eclass_member(userid)
,constraint UK_myPForP_tbl unique(university)
);
...
,constraint PK_myPForS_tbl PRIMARY KEY (mySSeq)
,constraint FK_myPForS_tbl foreign key(fk_subSeq) references subject_tbl(subseq)
,constraint FK_myPForP_tbl foreign key(fk_userid) references eclass_member(userid)
);

-- 학생 강의실 (마이페이지) (수정) ------------------------------------------------------ ★확인 필요
create table myPForS_tbl
(mySSeq  number not null -- 학생강의실 시퀀스
,fk_subSeq number not null -- 교과목번호 시퀀스
,fk_userid varchar2(50) not null -- 아이디
...
,constraint FK_myPForS_tbl foreign key(fk_subSeq) references subject_tbl(subseq)
,constraint FK_myPForP_tbl foreign key(fk_userid) references eclass_member(userid)
,constraint CK_myPForS_tbl check (status in('A', 'B', 'C', 'D', 'F'))
);




create sequence mySSeq
start with 1
increment by 1
nomaxvalue
nominvalue
...
nocache;
---------------------------- [[ 강의 테이블 ]] ---------------------------- ------------------------------------------------------ ★확인 필요
create table lecture_tbl -- 강의 테이블
(lecSeq number -- 시험문제번호
,subSeq number       -- 교과목 번호
,lecLink varchar2(1000) -- 강의영상
,lecStartday date -- 시청가능 시작일자
,lecEndday date -- 시청가능 마감일자
,constraint PK_lecture_tbl_lecSeq primary key(lecSeq)
,constraint FK_lecture_tbl_subSeq foreign key(subSeq)
                                  references subject_tbl(subSeq)
);
...
nocache;
---------------------------- [[ 강의 댓글 테이블 ]] ---------------------------- ------------------------------------------------------ ★확인 필요
create table lectureComment_tbl -- 강의 댓글 테이블
(leComSeq number -- 시험문제번호
,lecSeq number       -- 교과목 번호
,userid varchar2(50) -- 아이디
,comContent varchar2(2000) -- 댓글내용
,writedate date default sysdate -- 댓글작성일자
,constraint PK_lectureComment_leComSeq primary key(leComSeq)
,constraint FK_lectureComment_lecSeq foreign key(lecSeq)
                             references lecture_tbl(lecSeq)
,constraint FK_lectureComment_userid foreign key(userid)
                             references eclass_member(userid)
);
create sequence seq_leComSeq
start with 1
increment by 1
nomaxvalue 
nominvalue
...
,date           date default sysdate  not null   -- 작성일자
...
,date        date default sysdate  not null   -- 글쓴시간
,status         number(1) default 1   not null   -- 글삭제여부  1:사용가능한글,  0:삭제된글
...
,constraint FK_free_board_comment_userid foreign key(fk_userid)
                                    references eclass_member(userid)
,constraint FK_free_board_comment_parentSeq foreign key(parentSeq)
