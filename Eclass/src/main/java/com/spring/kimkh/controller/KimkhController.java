...
public class KimkhController {
	...
	public String SubjectMatter_insertEnd(LecutreMatterInsertVO lmiv, HttpServletRequest request) {
		 ...
		 * wtpwebapps\Board\resources String path = root+"resources" + File.separator
		 * +"files";
		 * 
		 * File.separator 는 운영체제에서 사용하는 폴더와 파일의 구분자이다. 운영체제가 windows 이라면 File.separator는
		 * "\"역슬레쒸이고, 운영체제가 UNIX,Linux 이라면 File.separator 는"/" 이다.
		 * 
		 * 
		 * //path가 첨부파일을 저장할 WAS(톰캣)의 폴더가 된다. System.out.println("~~~~~ 확인용 path => " +
		 * path);
		 * 
		 * 
		 * 
		 ...
		 * 
		 * try { bytes = attach.getBytes();//큰 파일을 절구통에 분해해서 lan선을 통해 내pc에서 was로 보낸다 //
		 * getBytes() 메소드는 첨부된 파일을 바이트단위로 파일을 다 읽어오는 것이다. // 예를 들어, 첨부한 파일이 "강아지.png"
		 * 이라면 // 이파일을 WAS(톰캣) 디스크에 저장시키기 위해 byte[] 타입으로 변경해서 올린다.
		 * 
		 * newFilename = fileManager.doFileUpload(bytes, attach.getOriginalFilename(),
		 * path); //위의 것이 파일올리기를 해주는 것읻 // attach.getOriginalFilename()은 첨부된 파일의
		 * 파일명(강아지.PNG)이다.
		 * 
		 ...
		 * 
		 * lmiv.setSaveSubImg(newFilename);//newFilename을 vo에 넣어준다 //was에 저장될
		 * 파일명(20192929292.png)
		 * 
		...
		//교수마이페이지에 insert시키기
		//int m = service.ForPInter();

		request.setAttribute("n", n);
		request.setAttribute("lmivList", lmivList);

		return "/subjectMatterList/SubjectMatterList.tiles1"; // 이게 뷰단페이지~!
